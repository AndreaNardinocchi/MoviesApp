...
// import { TVSeriesResponse } from "../types/interfaces";
...
//    */
//   const displayedMovies = series?.results ? filterFunction(series.results) : [];
...
import Spinner from "../components/spinner";
import { useQuery } from "react-query";
import { TVSeriesResponse, BaseTVSeriesProps } from "../types/interfaces";
import TVSeriesListPageTemplate from "../components/templateTVSeriesList";
import { useTranslation } from "react-i18next";
import useFiltering from "../hooks/useFiltering";
import { Pagination } from "@mui/material";
...
// Add explicit type for filter keys for better type safety
type FilterOption = "title" | "genre" | "release" | "sort";
...
const TVSeriesPage: React.FC = () => {
   ...
   * the pagination would not work
   */
  let displayedSeries: BaseTVSeriesProps[] = [];

  if (series?.results) {
    displayedSeries = filterFunction(series.results);

    // Add sorting by first_air_date (correct field for TV series)
    // https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort
    // https://stackoverflow.com/questions/74242074/sorting-array-of-objects-by-iso-date?
    displayedSeries.sort((a, b) => {
